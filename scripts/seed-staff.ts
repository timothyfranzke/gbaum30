...
const serviceAccount = JSON.parse(process.env.NEXT_FIREBASE_SERVICE_ACCOUNT_JSON || '{}');
...
async function seed() {
  for (const staff of STAFF) {
    const id = staff.name.toLowerCase().split(' ')[0];
    await db.collection('staff').doc(id).set(staff);
    console.log(`Seeded: ${staff.name} (${id})`);
  }
  console.log('Done!');
}
